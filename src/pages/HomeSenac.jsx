...
// Produtos via Supabase
const LANCHONETE_SENAC_ID = Number(import.meta.env.VITE_LANCHONETE_SENAC_ID ?? 7)

export default function HomeSenac(){
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try{
        ...
        if(mounted) {
          const tagged = data.map(p => ({ ...p, lanchoneteId: LANCHONETE_SENAC_ID }))
          setProducts(tagged)
        }
      ...
      <header className="relative bg-gradient-to-b from-blueTop to-[#5f88db] pt-4 pb-8 border-b border-black/20 shadow">
        <div className="max-w-6xl mx-auto px-4 relative">
          ...
          <div className="absolute right-2 top-2 bg-white rounded-md px-3 py-1 shadow border">
            <img src="/assets/Senac.png" alt="Senac" className="h-8" />
          </div>
          <div className="mt-3 flex justify-center">
            <div className="px-6 py-1.5 bg-white rounded-full shadow border text-[13px] tracking-wide">CARDAPIO SENAC</div>
          </div>
        </div>
      </header>
